...
class BookingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var timeSlotListenerRegistration: ListenerRegistration? = null
    ...
    fun fetchAvailableTimeSlots(
        ...
        timeSlotListenerRegistration = null
        ...
        viewModelScope.launch {
            try {
                ...
                // Attach real-time snapshot listener on day_slots ledger so slot availability stays in sync live
                timeSlotListenerRegistration = firestore.collection("day_slots").document(ledgerDocId)
                    .addSnapshotListener { ledgerSnap, e ->
                        if (e != null) {
                            Log.e(TAG, "❌ Error listening to day_slots: ${e.message}")
                            return@addSnapshotListener
                        }

                        val busyRanges = if (ledgerSnap != null && ledgerSnap.exists()) {
                        ...
                        Log.d(TAG, "📅 Real-time busy ranges updated: ${busyRanges.size} bays")

                        val updated = rawSlots.map { slot ->
                            var avail = true
                            val sm = slotToMinutes(slot.slot)
                        ...
                        Log.d(TAG, "📅 Real-time slots updated: ${updated.size} total, $availCount available")
